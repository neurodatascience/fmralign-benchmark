...
_BENCHMARK_INSTALL_MSG = 'See README for installation information.'
...
def _import_module_with_version_check(
    ...
    try:
        ...
        user_friendly_info = ('Module "{0}" could not be found. {1}').format(
            module_name,
            install_info or 'Please install it properly to use this benchmark.')
...
def _check_module_dependencies(is_benchmark_installing=False):
    """Throw an exception if benchmark dependencies are not installed.

    Parameters
    ----------
    is_benchmark_installing: boolean
    ...
    for (module_name, module_metadata) in REQUIRED_MODULE_METADATA:
        if not is_benchmark_installing:
